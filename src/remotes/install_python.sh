...
if [[ "$install_flag" == 1 ]]; then
	...
	if [[ "$(uname -s)" == "Linux" ]]; then
		...
		brew install curl wget git make gcc ncurses findutils sqlite openssl@3 readline zlib xz tcl-tk python@3 python@3.12 python-tk python-setuptools python-tabulate
